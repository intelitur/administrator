...
export class ContestsDetailsComponent implements OnInit {
  ...
  setData() {
    ...
    this.start_Date = new Date(
      this.datePipe.transform(this.contest.initial_date)
    );
      ...
      next: async (data: any) => {
        if (data.status == 200) {
          ...
          this.commonService.openSnackBar(
            `El concurso ${this.contest.name} ha sido cambiado`,
            "OK"
          );
          this.loading = false;
        } else {
  ...
  changeState({ source }: any) {
    if (this.contest.is_active) {
      this.commonService
        .confirmationDialog(
          `¿Desea eliminar el concurso: ${this.contest.name}?`
        )
        .then(async (result) => {
          if (result) {
            ...
            this.contetsService.changeStateContest(contest).subscribe({
              next: (data: any) => {
                ...
                if (data.status == 200) {
                  ...
                  this.commonService.openSnackBar(
                    `El concurso ${this.contest.name} ha sido eliminado`,
                    "OK"
                  );
                } else {
                  this.contest.is_active = !this.contest.is_active;
      ...
      this.commonService.openSnackBar(
        "No se puede reactivar un concurso",
        "OK"
      );
    }
  }
